...
#nr 4 cvs_ is a tidyverse (a tibble output)

2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
# The result shows that the object is a tibble created by tidyverse.
# class(kings) returns: "spec_tbl_df", "tbl_df", "tbl", "data.frame"

3. How many columns does the object have when created with these different functions?
# The dataset contains 10 columns (shown as 58 x 10 in the tibble output)
4. Show the dataset so that we can see how R interprets each column
# glimpse() shows how R interprets each column (character, numeric etc.)
glimpse(kings)
...
```{r}
kings_duration <- kings4 %>%
  mutate(duration = Regeringtids_Slut - Regeringtids_Start)
head(kings_duration)
```
...
```
Den gennemsnitlige regeringstid for danske konger i datasættet er ca. 19 år.
...
kings_duration %>%
  ...
  count()
```
24 danske konger regerede længere end gennemsnittet.
...
```{r eval = TRUE}
longest_monarch <- kings_duration %>% 
  filter(duration>mean(duration,na.rm = TRUE)) %>% 
...
```
De tre længst regenter var Erik d. 7 af Pommern med 15695 dage, Margrethe d. 2 med 18980 dage og Christian d. 4 med 21900 dage.
...
ggsave(here("figures","Monarch.png"), plot = Midyear_Monarch, height = 3, width = 6)
```
